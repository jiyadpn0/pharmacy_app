...
enum ReportType { detailed, summary, productVs, productVsSummary }
...
class _SalesReturnReportScreenState extends State<SalesReturnReportScreen> {
  ...
  void _search() async {
    ...
    final filteredReturns = await provider.fetchSaleReturnsInDateRange(_from, _to, loadItems: true, limit: _pageSize, offset: 0);
  ...
  void _loadMore() async {
    ...
    final provider = Provider.of<PharmacyProvider>(context, listen: false);
    final nextReturns = await provider.fetchSaleReturnsInDateRange(_from, _to, loadItems: true, limit: _pageSize, offset: _offset);
  ...
  List<dynamic> _processData(List<SaleReturnInvoice> returns) {
    switch (_reportType) {
      case ReportType.detailed:
        return returns.map((s) => {
          ...
          'patient': s.patient,
          'doctor': s.doctor,
          'original_invoice': s.originalInvoiceNo,
          'total': s.grandTotal,
        }).toList();
      case ReportType.summary:
        Map<String, Map<String, dynamic>> daily = {};
        for (var s in returns) {
          String day = DateFormat('dd/MM/yyyy').format(s.date);
          daily.putIfAbsent(day, () => {'date': day, 'total': 0.0, 'count': 0});
          daily[day]!['total'] += s.grandTotal;
          daily[day]!['count'] += 1;
        }
      ...
      case ReportType.productVsSummary:
        ...
        for (var s in returns) {
          for (var item in s.items) {
            ...
            prodSum.putIfAbsent(pName, () => {'product': pName, 'qty': 0, 'total': 0.0});
            prodSum[pName]!['qty'] += item.qty;
            prodSum[pName]!['total'] += item.total;
          }
        }
    ...
    switch (_reportType) {
      case ReportType.detailed:
        return [
          ...
          _ColInfo("entry_no", "Return No", flex: 1.0),
          _ColInfo("original_invoice", "Original Inv", flex: 1.0),
  ...
  void _printReport() async {
    ...
    final doc = await ReportPdfGenerator.buildReportPdf(
      title: "Sales Return Report (${_reportType.name.toUpperCase()})",
    ...
    if (!mounted) return;
    PrinterService.showProfessionalPreview(context: context, doc: doc, title: "Sales Return Report Preview");
  }
  ...
  Widget _buildReportHeader() {
    return Container(
      ...
      child: Text(
        "Sales Return Report From: ${DateFormat('dd/MM/yyyy').format(_from)} To: ${DateFormat('dd/MM/yyyy').format(_to)}",
  ...
  Widget _buildGridRow(Map<String, dynamic> row, List<_ColInfo> columns, List<double> scaledWidths, int index) {
    return InkWell(
      onTap: () {
        if (_reportType == ReportType.detailed || _reportType == ReportType.productVs) {
           Provider.of<AppProvider>(context, listen: false).openSalesReturn(entryNo: row['entry_no']);
        }
      },
      child: Container(
        height: 25,
