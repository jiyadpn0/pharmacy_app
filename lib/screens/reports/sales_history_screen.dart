...
class SalesHistoryScreen extends StatefulWidget {
  ...
  const SalesHistoryScreen({super.key, required this.productName, this.productId = "", this.windowId, this.hideHeader = false});
...
class _SalesHistoryScreenState extends State<SalesHistoryScreen> {
  ...
  List<_ColInfo> _getColumns() {
    return [
      _ColInfo("entry_no", "Entry No", width: 90, type: ColumnType.numeric),
      ...
      _ColInfo("qty", "Qty", width: 70, align: TextAlign.right, type: ColumnType.numeric),
      _ColInfo("mrp", "M.R.P", width: 80, align: TextAlign.right, type: ColumnType.numeric),
      _ColInfo("s_rate", "Rate", width: 80, align: TextAlign.right, type: ColumnType.numeric),
      _ColInfo("supplier_name", "Supplier", width: 180, type: ColumnType.text),
      _ColInfo("doctor", "Doctor", width: 130, type: ColumnType.text),
      _ColInfo("patient", "Patient", width: 130, type: ColumnType.text),
    ];
  }
  ...
  void didUpdateWidget(SalesHistoryScreen oldWidget) {
    ...
    if (oldWidget.productName != widget.productName || oldWidget.productId != widget.productId) {
      _loadHistory();
    }
  }
  ...
  void _loadHistory() {
    ...
    _historyFuture = pharmacy.getProductSaleHistory(widget.productName, productId: widget.productId);
  }
        ...
        builder: (context, snapshot) {
          ...
          _normalizedMaster = rawData.map((h) {
            ...
            return {
              ...
              'mrp': (h['mrp'] as num?)?.toDouble() ?? 0.0,
              's_rate': (h['s_rate'] as num?)?.toDouble() ?? 0.0,
              'supplier_name': h['supplier_name']?.toString() ?? 'N/A',
              'doctor': h['doctor']?.toString() ?? '',
              'patient': h['patient']?.toString() ?? '',
            };
          }).toList();
                                          ...
                                          itemBuilder: (context, index) {
                                            ...
                                            return GestureDetector(
                                              ...
                                              child: Container(
                                                height: 24,
                                                decoration: BoxDecoration(
                                                  color: isSelected ? const Color(0xFFDBEAFE) : (isEven ? Colors.white : const Color(0xFFF8FAFC)),
                                                  border: Border(bottom: BorderSide(color: isSelected ? Colors.blue.shade300 : const Color(0xFFF1F5F9)))
                                                ),
                                                child: Row(
                                                  children: [
                                                    _histCell(h['entry_no']?.toString() ?? '', _columns[0].width),
                                                    _histCell(h['type']?.toString() ?? 'Sales', _columns[1].width),
                                                    ...
                                                    _histCell(sRate.toStringAsFixed(2), _columns[7].width, align: TextAlign.right),
                                                    _histCell(h['supplier_name']?.toString() ?? 'N/A', _columns[8].width),
                                                    _histCell(h['doctor']?.toString() ?? '', _columns[9].width),
                                                    _histCell(h['patient']?.toString() ?? '', _columns[10].width, isLast: true),
                                                  ]
                                                )
                                              ),
                                            );
  ...
  void _tryOpenEntry(Map<String, dynamic> rowData) {
    ...
    if (entryFY.isNotEmpty && activeFY.isNotEmpty && entryFY != activeFY) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            "This sales entry ($entryNo) belongs to Financial Year: $entryFY.\n"
    ...
    try {
      ...
      debugPrint("Error opening sales entry: $e");
    }
  }
  ...
  Widget _histCell(String val, double width, {TextAlign align = TextAlign.left, Color color = Colors.black87, bool isLast = false}) {
    return Container(
      ...
      decoration: BoxDecoration(border: Border(right: BorderSide(color: isLast ? Colors.transparent : const Color(0xFFF1F5F9), width: 0.5))),
